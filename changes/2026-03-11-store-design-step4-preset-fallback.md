...
Además, el `PreviewHost` volvió a montar el `ThemeProvider` real dentro del iframe. Eso evita crashes de runtime en secciones como Contacto y FAQ, que esperan `theme.contact.*` y `theme.faqs.*` desde `styled-components`.
...
4. Ejecutar `npm run build` en la rama de deploy y verificar que no aparezcan errores de runtime por `theme.contact.containerBackground`.
...
- El `ThemeProvider` del preview usa el mismo armado runtime del storefront para evitar divergencia entre preview embebido y render real.