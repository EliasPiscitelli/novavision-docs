    ...
    **Flujo operativo actual (deploy sobre ramas feature):**
    - Ramas de deploy oficiales: `feature/automatic-multiclient-onboarding` (API/Admin) y `feature/multitenant-storefront` (Web).
    - Rama de integración: `develop` nace desde las ramas feature oficiales y no deploya.
    - Trabajo nuevo: PR desde la rama feature correspondiente hacia `develop`.
    - Validación: CI/QA se corre en `develop`.
    - Propagación a producción: cherry-pick desde `develop` hacia la(s) rama(s) feature que se deployan.
    - Nuevos cambios que nazcan en una rama feature vuelven a entrar por PR a `develop`.
    - Solo cuando se quiera un release “bendecido”, se hace PR de `develop` → `main` (no obligatorio ahora).
