...
### Nuevos Archivos Creados (apps/admin)

| Archivo | Propósito |
|---------|-----------|
| `src/utils/onboarding/onboardingRoutesMap.ts` | Single source of truth para mapeo estado→ruta |
| `src/utils/onboarding/onboardingRouteResolver.ts` | Resolver centralizado con sanitización de `next` param |
| `src/utils/onboarding/useOnboardingGuard.ts` | Hook para aplicar guardias en componentes |
| `src/utils/onboarding/index.ts` | Barrel export del módulo |
| `src/utils/OnboardingGuardedRoute.jsx` | Route wrapper para React Router |
| `src/utils/onboarding/__tests__/onboardingRouteResolver.spec.ts` | Tests unitarios |

### Archivos Modificados (apps/admin)

| Archivo | Cambio |
|---------|--------|
| `src/utils/resolvePostLoginRoute.js` | Convertido a wrapper de compatibilidad que delega al nuevo resolver |
| `src/context/AuthContext.jsx` | Import actualizado para usar nuevo resolver |
| `src/pages/ClientLogin/index.jsx` | Usa resolver centralizado + sanitizeReturnPath |
| `src/pages/OAuthCallback/index.jsx` | Usa resolver centralizado |
| `src/components/Header/index.jsx` | Botón "Mi Cuenta" usa `getCanonicalRoute()` |
| `src/App.jsx` | Rutas de onboarding ahora usan `OnboardingGuardedRoute` |
